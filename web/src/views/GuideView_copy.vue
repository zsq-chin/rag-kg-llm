...
import GuideChatComponent from '@/components/GuideChatComponent.vue'
import { MessageSquareMore, PanelLeftClose } from 'lucide-vue-next'

const convs = reactive(JSON.parse(localStorage.getItem('guide-convs')) || [
  {
    id: 0,
    title: '新对话',
    history: [],
...
const state = reactive({
  isSidebarOpen: JSON.parse(localStorage.getItem('guide-sidebar-open') || 'true'),
})
...
watch(
  () => state.isSidebarOpen,
  (newValue) => {
    localStorage.setItem('guide-sidebar-open', JSON.stringify(newValue))
  }
)
...
watch(
  () => convs,
  (newStates) => {
  localStorage.setItem('guide-convs', JSON.stringify(newStates))
  },
  { deep: true }
)
...
onMounted(() => {
  const savedSonvs = JSON.parse(localStorage.getItem('guide-convs'))
  if (savedSonvs) {
