  ...
  >
    <template #description>
      <div class="database-info">
        ...
        <a-tag color="green" v-if="database.dimension">嵌入维度：{{ database.dimension }}</a-tag>
        <span class="row-count">文件数量：{{ database.files ? Object.keys(database.files).length : 0 }} 文件 · 知识库 ID：{{ database.db_id }}</span>
      </div>
    </template>
    <!-- <template #actions>
  ...
  <div class="db-main-container">
    <a-tabs v-model:activeKey="state.curPage" class="atab-container" type="card">
        ...
        <div class="db-tab-container">
          <div class="actions" style="display: flex; gap: 10px; justify-content: space-between;">
            <div class="left-actions" style="display: flex; gap: 10px;">
              ...
              <a-button :icon="h(ReadOutlined)">点击文件可查看分块</a-button>
            </div>
              ...
              <div class="upload" v-if="uploadMode === 'file'">
                ...
                >
                  ...
                  <p class="ant-upload-hint">
                    目前支持上传文本文件，如 .pdf, .txt, .md .html。且同名文件无法重复添加
                  </p>
                </a-upload-dragger>
              </div>
      ...
      <a-tab-pane key="query-test" force-render>
        <template #tab><span><SearchOutlined />检索召回测试</span></template>
...
const databaseId = ref(props.database_id);
const database = ref({});
