...
<!--        <a-tag color="blue" v-if="database.embed_model">{{ database.embed_model.split("/").slice(-1)[0]}}</a-tag>-->
<!--        <a-tag color="green" v-if="database.dimension">{{ database.dimension }}</a-tag>-->
        <span class="row-count">{{ database.files ? Object.keys(database.files).length : 0 }} 文件 </span>
      </div>
    </template>
    <template #actions>
  ...
  <div class="db-main-container">
    <a-tabs v-model:activeKey="state.curPage" class="atab-container" type="card">
        ...
        <div class="db-tab-container">
          ...
          <a-table
            ...
            <template #bodyCell="{ column, text, record }">
              <!-- 鼠标悬停提示框 -->
              <a-tooltip v-if="column.key === 'filename'" :title="record.file_id" placement="right">
                <a-button class="main-btn" type="link" @click="openFileDetail(record)">{{ text }}</a-button>
              </a-tooltip>
              <span v-else-if="column.key === 'file_type'" :class="['span-type', text]">{{ text.toUpperCase() }}</span>
      ...
      <a-tab-pane key="add-file">
        <template #tab><span><FileAddOutlined />添加文件</span></template>
              ...
              <div class="upload" v-if="uploadMode === 'file'">
                ...
                >
                  ...
                  <p class="ant-upload-hint">
                    目前仅支持上传文本文件，如 .pdf, .txt, .md .html .json .csv。且同名文件无法重复添加
                  </p>
                </a-upload-dragger>
              </div>
      ...
      <a-tab-pane key="query-test" force-render>
        <template #tab><span><SearchOutlined />检索文件内容</span></template>
              ...
              <div class="query-params" v-if="state.curPage == 'query-test'">
                ...
                <div class="params-group">
                  <div class="params-item">
                    <p>检索数量：</p>
                    <a-input-number size="medium" v-model:value="meta.maxQueryCount" :min="1" :max="20" />
                  </div>
                  <!-- <div class="params-item">
                    <p>过滤低质量：</p>
                  ...
                  <div class="params-item">
                    <p>筛选 TopK：</p>
                    <a-input-number size="medium" v-model:value="meta.topK" :min="1" :max="meta.maxQueryCount" />
                  </div>
                  <div class="params-item" v-if="configStore.config.enable_reranker">
                    ...
                    <a-radio-group v-model:value="meta.sortBy" button-style="solid" size="small">
                      <a-radio-button value="rerank_score">重排序</a-radio-button>
                  ...
                  <div class="params-item col">
                    ...
                    <a-segmented v-model:value="meta.use_rewrite_query" :options="use_rewrite_queryOptions">
                      <template #label="{ payload }">
                        <div>
                          <a-tooltip v-if="payload.subTitle === '伪文档生成'" placement="right">
                            <template #title>
                              伪文档生成是利用模型根据用户的原始 query 和上下文，生成一段“假设性的、可能存在的回答文档”，
                              用这段文档作为查询输入去做向量检索（RAG）或用于 query 重写。
                            </template>
                            <p style="margin: 4px 0">{{ payload.subTitle }}</p>
                          </a-tooltip>
                          <p v-else style="margin: 4px 0">{{ payload.subTitle }}</p>
                        </div>
                      </template>
                    </a-segmented>
                  </div>
          ...
          <div class="query-result-container">
            ...
            <div class="query-test" v-if="queryResult">
              ...
              <div class="query-result-card" v-for="(result, idx) in (filteredResults)" :key="idx">
                <p>
                  <strong>#&nbsp;{{ idx + 1 }}&nbsp;&nbsp;&nbsp;</strong>
                  <span><strong>类型</strong>：{{ result.file.file_type }}&nbsp;&nbsp;&nbsp;<strong>名称</strong>：{{ result.file.filename }}&nbsp;&nbsp;&nbsp;</span>
...
const columns = [
  { title: '文件ID', dataIndex: 'file_id', key: 'file_id' ,width: 110},
    ...
    .sider-top {
      .query-params {
        ...
        .params-title {
          ...
          padding-bottom: 8px; /* 给文字和边框之间加一点距离 */
        }
