...
import WriterChatComponent from '@/components/WriterChatComponent.vue'
import { MessageSquareMore, PanelLeftClose } from 'lucide-vue-next'
import { writerRecordApi } from '@/apis/auth_api'
...
const state = reactive({
  isSidebarOpen: JSON.parse(localStorage.getItem('writer-sidebar-open') || 'true'),
...
watch(
  () => state.isSidebarOpen,
  (newValue) => {
    localStorage.setItem('writer-sidebar-open', JSON.stringify(newValue))
  }
)
...
const delConv = async (index) =>  {
  try {
    await writerRecordApi.deleteChatRecord(convs[index].id)
  } catch (e) {
...
const loadChatRecords = async () => {
  try {
    const res = await writerRecordApi.getChatRecords()
    if (res.length !== 0) {
...
const saveChatRecords = async (changedConv) => {
  try {
    await writerRecordApi.saveChatRecords(changedConv)
  } catch (e) {
    message.error('保存聊天记录失败')
  }
}
