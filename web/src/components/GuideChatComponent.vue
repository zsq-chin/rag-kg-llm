...
import { Ellipsis, PanelLeftOpen, MessageSquarePlus, Search ,Compass, Waypoints, BookCheck } from 'lucide-vue-next'
...
const meta = reactive(JSON.parse(localStorage.getItem('guide-meta')) || {
  use_graph: false,
  use_web: false,
  graph_name: "neo4j",
  selectedKB: null,
...
const fetchChatResponse = async (user_input, cur_res_id) => {
  ...
  // 异步获取会议事项层级结构
  let meetingStructure = {};
  try {
    const response = await fetch('/会议事项层级结构.json');
    if (response.ok) {
      meetingStructure = await response.json();
    }
  } catch (error) {
    console.error('加载会议事项层级结构失败:', error);
  }

  // 构建系统提示词
  const systemPrompt = `你是一个学院议事规则专家，需要根据提供的《会议事项层级结构》判断用户提出的议题应该提交给哪个会议（党委会或党政联席会议）以及相应的决策类型（讨论决定、研究落实、先行把关等）。请严格按照以下规则进行分析：

1. 首先确定议题所属的大类（如党的建设、改革发展稳定等）
2. 然后找到所属子类，最终匹配的具体事项
3. 最后根据规则给出会议类型和决策类型
4. 需留意，部分具体事项可能需提交至多个会议审议，所有涉及的会议都要完整纳入，逐一梳理清楚 。
5. 你的输出应该严格按照示例中的格式，不要输出多余内容
...
watch(
  () => meta,
  (newMeta) => {
    localStorage.setItem('guide-meta', JSON.stringify(newMeta));
  },
  { deep: true }
);
