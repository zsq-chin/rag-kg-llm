...
# 一、初始化与配置相关方法
# 1. __init__(): 初始化知识库，设置工作目录、缓存、默认参数，检查数据迁移，加载模型
# 2. _load_models(): 加载嵌入模型和重排序模型，连接Milvus向量数据库
# 3. _check_migration(): 检查并执行从JSON格式到SQLite的迁移
# 4. restart(): 重新加载模型

# 二、数据库操作方法
# 1. create_database_record(): 创建知识库记录
# 2. get_all_databases(): 获取所有知识库
# 3. get_database_by_id(): 根据ID获取知识库
# 4. update_database_record(): 更新知识库记录
# 5. delete_database_record(): 删除知识库记录
# 6. add_file_record(): 添加文件记录
# 7. update_file_status(): 更新文件状态
# 8. delete_file_record(): 删除文件记录
# 9. get_files_by_database(): 获取知识库下的所有文件
# 10. get_file_by_id(): 根据ID获取文件
# 11. add_node(): 添加知识块节点
# 12. get_nodes_by_file(): 获取文件下的所有知识块
# 13. get_nodes_by_filter(): 根据条件筛选知识块

# 三、文件处理与索引方法
# 1. save_files_for_pending_indexing(): 保存待索引文件并分块
# 2. save_urls_for_pending_indexing(): 保存待索引URL内容并分块
# 3. trigger_file_indexing(): 触发文件索引过程
# 4. delete_file(): 删除文件及其关联数据
# 5. delete_database(): 删除整个知识库

# 四、检索查询方法
# 1. query(): 执行查询并返回结果
# 2. get_retriever_by_db_id(): 获取指定知识库的检索器
# 3. get_retrievers(): 获取所有知识库的检索器
# 4. search(): 执行向量搜索
# 5. search_by_vector(): 通过向量执行搜索
# 6. examples(): 获取知识库示例
# 7. search_by_id(): 通过ID搜索

# 五、Milvus向量数据库操作方法
# 1. connect_to_milvus(): 连接Milvus数据库
# 2. get_collection_names(): 获取集合名称列表
# 3. get_collections(): 获取所有集合信息
# 4. get_collection_info(): 获取集合详细信息
# 5. add_collection(): 添加新集合

# 六、辅助方法
# 1. _to_dict_safely(): 安全地将对象转换为字典
# 2. _ensure_db_folders(): 确保知识库文件夹存在
# 3. get_db_upload_path(): 获取上传路径
# 4. _mark_waiting_files_as_failed(): 将等待状态文件标记为失败
# 5. check_embed_model(): 检查嵌入模型是否匹配

# 七、工具函数
# 1. parse_node_data(): 解析节点数据
# 2. gen_filename_from_url(): 从URL生成文件名
...
class KnowledgeBase:
    ...
    def file_Handle(
            ...
            external_api_url: str = "http://202.204.194.41:8076/api/v1/tasks/submit"  # 替换为实际地址
    ):
            ...
            try:
                with open(file_path, "rb") as f:
                    ...
                    if file_suffix in ['.mp3', '.wav', '.ogg', '.flac']:
                        ...
                        data["lang"] = "ch"  # 音频也指定中文
                    elif file_suffix in ['.mp4', '.avi', '.mov', '.mkv']:
                        data["backend"] = "video"
                        data["lang"] = "ch"  # 视频也指定中文
                    else:
                        data["backend"] = "pipeline"  # 文档/图片用pipeline

                    # 发送请求（超时延长到180秒，适配表格/公式识别）
                    ...
                    print(f"✅ 成功提交文件: {file_path.name}")
                    print(f"   任务ID: {result.get('task_id')}")
                    print(f"   后端: {data['backend']} | 语言: {data['lang']}\n")

            except Exception as e:
                print(f"❌ 提交文件 {file_path.name} 失败: {str(e)}\n")
    ...
    def query(self, query_text, db_id, **kwargs): # Renamed 'query' to 'query_text' to avoid clash
        ...
        return {
            "results": db_result_filtered, # 已过滤且经过重排序
